...
export default function ReservationInfoModal({
  ...
  return (
    <div className='rounded-8 absolute top-0 right-0 z-50 h-697 w-429 border border-gray-200 bg-white shadow-lg'>
      {/* 헤더 */}
        ...
        <div className='mt-27'>
          ...
          <div className='mt-24'>
            <Dropdown
              ...
              onChange={(value) => {
                const scheduleId = getScheduleIdFromTimeSlot(value, schedules);
                setSelectedScheduleId(scheduleId);
              }}
              placeholder='시간대를 선택하세요'
              className='h-41 w-381'
            />
          </div>
        </div>
          ...
          {selectedScheduleId && allReservations.length > 0 ? (
            <div className='space-y-12'>
              {allReservations.map((reservation) => (
                <div
                  key={reservation.id}
                  className='rounded-8 flex h-116 w-full items-end justify-between border border-gray-200 p-12'
                >
